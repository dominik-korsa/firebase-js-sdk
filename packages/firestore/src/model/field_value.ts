...
export class ObjectValue {
  ...
  /** Returns a new Builder instance that is based on an empty object., using the Platform's default setting for `useProto3Json` */
  static newBuilder(): ObjectValueBuilder {
    return ObjectValue.EMPTY.toBuilder();
  }

  /**
   * Returns the backing Protobuf representation.
   *
   * Asserts that `useProto3Json` matches the value passed at ObjectValue
   * construction. This ensures that all Protos handled by the SDK are in the
   * same format (Proto3Json for the Web build, ProtoJS for the Node build).
   */
  toProto(): api.Value {
    return this.proto;
  }
